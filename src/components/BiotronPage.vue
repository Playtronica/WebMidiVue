<template>
  ...
  <GroupOfCommands name-of-group="BPM">
    <template v-slot:objects>
      ...
      />
      <SliderCommand command-label="Note Off Percent" :max-value="100" :default-value="100"/>
      <SliderCommand command-label="Plant Bpm" :max-value="30" :default-value="4"/>
    </template>
  </GroupOfCommands>
  <GroupOfCommands name-of-group="Sensitivity (fib)">
    <template v-slot:objects>
      <SliderCommand command-label="Note Distance" :max-value="100" :default-value="50"/>
      <SliderCommand command-label="First Value" :max-value="100" :default-value="10"/>
    </template>
  </GroupOfCommands>
  ...
  <button @click="console.log(this.commands_data.plantBpm.value)"></button>
</template>
