<template>
  <div @keyup.enter="this.sendData">
    ...
    <GroupOfCommands name-of-group="BPM" description="Plant BPM - how many notes from the plant will be generated per minute
...
Light Bpm - once every number of notes from the plant the note from the photoresistor will sound">
    ...
    <GroupOfCommands name-of-group="Sensitivity (fib)" description="Sensitivity (fib) - Fibonacci parameter responsible for the note distribution curve">
      <template v-slot:objects>
    ...
    <GroupOfCommands name-of-group="Smoothness" description="Smoothness - the smoothness of the notes played, where 0 is an instant change in notes, 99 is a smooth change (notes change over time)">
      <template v-slot:objects>
        <SliderCommand :key="this.forceRerender" :command-object="this.commands_data.smoothness" />
      </template>
    </GroupOfCommands>
    <GroupOfCommands name-of-group="Scale" description="Scale - scale played from the device">
      <template v-slot:objects>
        <SelectCommand :key="this.forceRerender" :list-of-variants="this.scales" :command-object="commands_data.scale"/>
      </template>
    </GroupOfCommands>
    <GroupOfCommands name-of-group="Velocity" description="Velocity - pressing force
Humanize - Velocity randomization at a controlled interval
Mute - disable note generation from the channel
    ">
      <template v-slot:objects>
        <GroupOfCommands name-of-group="Plant" description="Plant - responsible for the notes generated by the plant">
          <template v-slot:objects>
            <div class="row">
              <div class="col">
        ...
        <GroupOfCommands name-of-group="Light" description="Light - responsible for notes generated from the photoresistor">
          <template v-slot:objects>
            <div class="row">
              <div class="col">
    ...
    <GroupOfCommands name-of-group="Ultra sensitivity" description="Ultra sensitivity - Increases the sensitivity of generation from a plant">
      <template v-slot:objects>
        <DisableCommand :key="this.forceRerender" :command-object="commands_data.randomness"/>
      </template>
    </GroupOfCommands>
    <GroupOfCommands name-of-group="Same Note" description="Same Note - notes that are played only when changing notes with a customizable step, where 1 - produces a note if the notes have changed by 1 note, 10 if there has been a shift by 10 notes">
      <template v-slot:objects>
        <SliderCommand :key="this.forceRerender" :command-object="commands_data.same_note" />
      </template>
    </GroupOfCommands>
    <GroupOfCommands name-of-group="Light Notes Range" description="Light Notes Range - setting the range of notes played from the photoresistor (lower and upper limits are set)">
      <template v-slot:objects>
        <SliderRangeCommand
    ...
    <GroupOfCommands description="Send - sending settings to the device
...
Drop Preset Here - you need to transfer the file there by drag drop, or by clicking on the button, select the settings file.">
      <template v-slot:objects>
