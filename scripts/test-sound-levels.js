...
;(async () => {
  ...
  try {
    ...
    const metrics = await page.evaluate(async () => {
      ...
      return {singleNotes, denseChords}
    })

    for (const metric of [...metrics.singleNotes, ...metrics.denseChords]) {
    ...
    assert(Math.min(...metrics.singleNotes.map(metric => metric.peak)) >= 0.18,
