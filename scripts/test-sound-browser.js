...
async function verifyCapabilityFallbacks(browser, origin) {
  ...
  await audioOnly.getByText(/Chrome or Edge on a computer or Android phone/i).waitFor()
  ...
  await iphone.getByText(/MIDIWeb Browser/i).waitFor()
...
;(async () => {
  ...
  try {
    ...
    const deviceMenu = page.getByRole('navigation', {name: 'Choose a device'})
    assert.strictEqual(await deviceMenu.count(), 1)
